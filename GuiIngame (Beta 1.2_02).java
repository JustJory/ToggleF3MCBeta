...
public class GuiIngame extends Gui {
	...
	public void renderGameOverlay(float var1, boolean var2, int var3, int var4) {
		...
		if (Keyboard.isKeyDown(Keyboard.KEY_F3)) {
			if (!f3KeyPressed) {
				showDebugInfo = !showDebugInfo; // Toggle the boolean value
			}
			f3KeyPressed = true;
		} else {
			f3KeyPressed = false;
		...
		if(showDebugInfo) {
			...
			this.drawString(var8, "x: " + String.format("%.1f", this.mc.thePlayer.posX), 2, 64, 14737632);
			this.drawString(var8, "y: " + String.format("%.1f", this.mc.thePlayer.posY), 2, 72, 14737632);
			this.drawString(var8, "z: " + String.format("%.1f", this.mc.thePlayer.posZ), 2, 80, 14737632);
		} else {
